...
| **Benchmarks** (`benchmarks/`) | 10 casos builtin executáveis via `izanagi benchmark` + `compare` entre builds, mais o Token Benchmark (`benchmark tokens`): legado vs Commander em chamadas, tokens e custo do PLANO, de forma determinística. `benchmark run --execute --compare` roda a mesma suíte pelos dois caminhos de planejamento, com o mesmo teto por caso, e mede oito dimensões de EXECUÇÃO: tokens, custo, latência, model calls, agent calls, retries, taxa de sucesso e taxa de verificação. Plano e execução nunca dividem o mesmo campo. |
