...
import { maestroCommand } from './commands/maestro.js';
...
export async function runCLI(args: string[]): Promise<void> {
  ...
  switch (command) {
    ...
    // Ponte com o RunMaestro (executor externo de agentes): planeja com o
    // Commander, traduz o grafo em Auto Run docs (.md + markers) e despacha
    // via maestro-cli. Subsistema canvas removido na v3.24.0 (ADR 2026-09-22).
    case 'maestro':
      await maestroCommand(baseDir, rest, stateDir);
      break;
...
\x1b[1mUsage:\x1b[0m
  ...
  \x1b[32mmaestro init|plan|run|goal|list\x1b[0m  Maestro Bridge (RunMaestro executor)
