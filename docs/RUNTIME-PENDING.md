...
- **`references-exist` mede LUGAR, não semântica.** Confere que o diretório citado existe; não confere que a função citada exista dentro do arquivo. Isso exigiria análise sintática por linguagem — o `python-engine/ast_analyzer` já faz parte disso, e ligá-lo à verificação é trabalho de outra fase.
