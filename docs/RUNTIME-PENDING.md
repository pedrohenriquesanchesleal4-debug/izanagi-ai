...
**Rodada seguinte, mesmo método (2026-09-10):** com o `direct` fechando, o `--mode orchestrated` foi rodado de verdade e caíram mais seis defeitos, um escondendo o outro — artefato que era transcrição de ferramenta ENCENADA (com dois arquivos inexistentes citados) e teria sido entregue como resposta no kind `raw`; prompt que não dizia o contrato que a verificação cobrava; `not-contains "TODO"` reprovando "todos" (terceira instância da mesma falha); fase `recovery` sem orçamento para uma retentativa, o que tornava o healing decorativo; teto contando só nós de kind `agent` e ignorando o `evaluator`; e recusa do CLI hospedeiro tratada como falha retentável. Todos no `CHANGELOG.md`, com teste.

**Aberto, e é o item mais valioso da próxima rodada: um run que produziu trabalho verificado entrega ZERO arquivo quando um critério não fecha.** Medido em 2026-09-10, depois de todos os consertos acima: modo `orchestrated`, cinco artefatos válidos (relatório de segurança de 7,4KB, análise de 13,7KB, remediação de 23,7KB), score 0.938, orçamento sobrando (112.256 de 260.000), custo pago de US$ 0,69 — e `tmp/verify-orch/` vazio, porque um nó não comprovou um critério em duas tentativas e o grafo abortou antes do `deliver`. O veredito `HUMAN_REQUIRED` está CERTO (o runtime não deve declarar sucesso sem prova); errado é o que ele faz com o trabalho já comprovado. O nó `deliver` custa zero token: entregar o que é válido, marcado como parcial e dizendo qual critério não fechou, é estritamente melhor que descartar tudo. Perder 100% da entrega por 1 critério de N é a troca errada.
