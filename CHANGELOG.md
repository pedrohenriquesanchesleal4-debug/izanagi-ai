...
- **Um artefato podia ser uma transcrição de ferramenta ENCENADA, e ser entregue como resposta.** Com `--agent-tools none` (nenhuma tool no subprocesso), o nó de auditoria gravou onze linhas no formato `**Tool Call: rg -il "jwt"**` / `Status: Completed` / `Terminal:`, citando `src/runtime/security/tokens.ts` e `webhookSecurity.ts` — dois arquivos que não existem neste repositório. Sem tools o modelo não leu nada: ele encenou a leitura. Só o schema de `security-report` pegou, por exigir estrutura; no kind `raw`, que não exige campo nenhum, a encenação teria sido gravada no `--output` como a resposta do run. Agora `validateArtifact` reprova artefato cuja narração de ferramenta é o texto inteiro (regra de PROPORÇÃO: uma resposta legítima pode citar um comando, não pode ser só a citação), e o contrato do executor sem tools passa a dizer explicitamente para não emitir transcrição nem inventar conteúdo de arquivo.
- **O prompt não dizia o contrato que a verificação ia cobrar.** A linha era "Artefato esperado deste nó: `security-report` (conteúdo estruturado)" e a verificação exigia `severity`, `vulnerabilities`, `remediation` e 300 caracteres. O nó falhou nas duas tentativas por "4 critério(s) obrigatório(s) reprovado(s)", escrevendo sobre o assunto certo na única forma que podia adivinhar. `artifactRequirementLine` deriva a exigência do próprio `ARTIFACT_SCHEMAS`: cobrar um contrato que não foi comunicado não mede o agente, mede a sorte.
- **`not-contains "TODO"` reprovava português, terceira instância da mesma falha.** O check é case-insensitive por default, então "todos os endpoints" num relatório correto disparava "saída sem TODO (zero stub/checklist)". Novo campo `wholeWord` em `contains`/`not-contains`, aplicado a marcadores (`/^[A-Z]{3,}$/`) junto de `caseSensitive`; termo que não é palavra (`"// implement later"`, `"[ ] checklist"`) segue como substring, que é a única busca que faz sentido para ele. A mesma regra foi para o `forbidden` de `validateArtifact`.
- **O healing era decorativo: a fase `recovery` não pagava uma retentativa.** Medido: `recovery` fechou em 16.420/16.420 tentando reexecutar um nó de ~20.000 tokens. O runtime decidia curar, tentava, e morria no orçamento antes de chamar o modelo. Duas causas: a fase `planning` reservava de 5% a 15% do teto e **nunca gasta um token** (o Commander é determinístico), enquanto `recovery` ficava com 10%; e o piso do executor dimensionava só a `execution`. Os pesos foram rebalanceados para onde o gasto acontece de fato, e o piso ganhou uma segunda conta (`minTokensPerRetry`) que só se aplica onde o modo permite retentar.
- **O teto contava só nós de kind `agent`.** Um grafo orchestrated tem também `evaluator`, que chama modelo: o run recebeu teto para 3 nós tendo 4, e morreu no último (124.553 de 117.000) depois de quase todo o trabalho feito. Agora conta todo nó que não seja `tool`, `gate` ou `approval` — os três que o orchestrator resolve sem chamar modelo.
- **Recusa do CLI hospedeiro virava 400 caracteres de JSON cru e queimava retentativa.** Um processo que termina com `duration_api_ms: 0` e uso zero não chegou à API: é quota, autenticação ou indisponibilidade, e retentar não devolve nenhuma das três. `describeFailure` lê o envelope e diz isso em uma linha; `classifyFailure` marca como `non-recoverable`, antes da regra de `tool` que casaria com "exit code" e mandaria retentar.

### Fixed
- **`izanagi run "..."` sem flag nenhuma não entregava nada pelo caminho sem API key.** O executor `claude-cli` existia e funcionava desde a v3.22.0, mas o comando mais básico do framework terminava em `HUMAN_REQUIRED` com "teto de tentativas esgotado" e zero arquivo gravado. Medido rodando de verdade, não inferido. Três defeitos empilhados, cada um escondendo o seguinte:

  1. **O teto do run era de outra era.** `budgetForMode('direct')` dá 2.000 tokens; um nó deste executor foi medido em ~20.000, porque o CLI hospedeiro cobra o próprio system prompt (4.095 tokens) em toda chamada. Os dois números nunca tinham sido reconciliados: o aviso de orçamento só disparava para quem passasse `--budget` baixo à mão, e quem não passava nada caía num teto ainda menor. Novo `CommanderInput.minTokensPerNode`, derivado do executor detectado em `buildExecutionPlan` e aplicado ao teto do run e ao contrato de cada nó que CONSOME modelo (nó de tool é determinístico e continua em zero). `--budget` explícito continua vencendo: um teto que o usuário declarou é decisão dele.
  2. **O piso recomendado era a média medida.** 30.000 vinha de dividir 18.000 pela fatia da fase `execution` (0,65). Média não é piso: execuções do mesmo objetivo gastaram 19.799, 20.706 e 20.114 tokens, e a fatia varia com a complexidade (0,7 / 0,65 / 0,6). `recommendedBudget` passou a ser calculado: consumo medido, mais folga de variância declarada (`AGENT_CLI_VARIANCE_HEADROOM`), dividido pela MENOR fatia possível (`MIN_EXECUTION_PHASE_SHARE`). Piso: **39.000** sem tools, **148.000** com `read`. Trocar uma falha por orçamento por outra não seria conserto.
  3. **A varredura anti-stub reprovava português.** `/TODO/i` casava como substring, então "todo", "toda" e "todos" marcavam o artefato como código inacabado: uma implementação correta de `validarCPF`, com testes, foi recusada porque um comentário dizia "rejeita CPFs com todos os dígitos iguais". Num framework escrito em pt-BR isso não é caso de borda, é o caso comum. Os três marcadores viraram `\bTODO\b`, `\bFIXME\b` e `\bXXX\b`, com fronteira de palavra e sensíveis a caixa: marcador de stub é escrito em caixa alta por convenção, então nada de detecção real se perde.

  Verificado de ponta a ponta na mesma máquina, sem chave, sem modelo local e sem flag: `PASS`, score 1, entrega gravada em disco, 20.114 tokens de 39.000, custo medido $0.0326.

### Fixed (escolha de agente)
