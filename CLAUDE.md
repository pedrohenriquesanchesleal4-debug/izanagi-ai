...
- `brainstorming` — Transforma uma ideia bruta em design/spec completo por entrevista dirigida em 3 fases (~15 perguntas, uma por vez) +…
- `deep-research` — Pesquisa profunda em múltiplas fontes na web: gera plano de busca, executa múltiplas queries, coleta, sintetiza e…
- `ui-ux-pro-max` — Design intelligence profissional para UI/UX: gera design system completo…
- `motion-design` — Skill de Motion Design para Web — escolha e uso correto de bibliotecas de animação: GSAP (ScrollTrigger, SplitText),…
- `animation-web` — Skill de Web Animation Cinematográfica — scrollytelling, scroll-driven animations, scroll image sequences…
- `webgl-3d` — Skill de 3D na Web — Three.js, React Three Fiber, WebGL, shaders (GLSL), scroll-driven 3D, partículas, modelos GLTF e…
- `frontend` — Skill de frontend para o Izanagi. Contém todos os design tokens do Tailwind CSS, padrões de design identificados nas…
- `tdd` — Test-Driven Development com Iron Law: escreva o teste antes, veja falhar, código mínimo para passar, refatore. Use em…
- `security-privacy` — Skill de Seguranca e Privacidade para o Izanagi. Aborda OWASP Top 10, LGPD/GDPR, seguranca de APIs, authentication,…
- `qa` — Skill de Quality Assurance para o IzanagiAI. Contém checklist completo de qualidade de código, validações de…
- `memoria-projeto` — Mantém memória persistente do projeto entre sessões, guardando decisões, padrões de código e erros já resolvidos,…
- `economia-tokens` — Reduz o consumo de tokens em QUALQUER tarefa de código ou análise de arquivos. Use sempre, em toda tarefa — não é…
- `handoff-sessao` — Grava um resumo curto do estado da tarefa em andamento para retomar na próxima sessão sem perder contexto e sem…
