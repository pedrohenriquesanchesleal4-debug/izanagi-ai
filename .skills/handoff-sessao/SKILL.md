---
name: "handoff-sessao"
description: "Grava um resumo curto do estado da tarefa em andamento para retomar na próxima sessão sem perder contexto. Use quando o usuário disser 'pausa'/'continuo depois' ou a conversa estiver perto do limite de contexto. Gatilhos de ativação: handoff de sessão; quando acionar; o que gravar (arquivo: `.agents/memoria/em-andamento.md`); <nome da tarefa> — <data iso>."
version: 2.0.0
category: docs
tools:
  mcp:
...
> Migrado deterministicamente de `skills/handoff-sessao/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Documentação & Comunicação (`docs`)
- **Resumo:** Grava um resumo curto do estado da tarefa em andamento para retomar na próxima sessão sem perder contexto.
- **Ativar quando:** Use quando o usuário disser 'pausa'/'continuo depois' ou a conversa estiver perto do limite de contexto.
- **Escopo canônico:** Handoff de Sessão
- **Seções do corpo original:** Quando acionar · O que gravar (arquivo: `.agents/memoria/em-andamento.md`) · <nome da tarefa> — <data ISO> · Migração pg para mysql — 2026-08-10 · Regras
...
### Passo 1 — Leia .agents/memoria/em-andamento.md (existe?

**Leia** `.agents/memoria/em-andamento.md` (existe? qual tarefa está ativa?).

### Passo 2 — Atualize/sobrescreva a entrada da tarefa atual com o formato acima.

**Atualize/sobrescreva** a entrada da tarefa atual com o formato acima.

### Passo 3 — Confirme ao usuário:

**Confirme** ao usuário: "Handoff gravado — ao retomar, começo lendo o estado."
