...
description: "Use após corrigir um bug para investigar a causa raiz sistêmica (5 Whys, Fishbone, Premortem) e recomendar mudanças de processo. Gatilhos de ativação: skill: root cause analyzer; identity; goals; triggers."
version: 2.0.0
...
> Migrado deterministicamente de `skills/root-cause-analyzer/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Engenharia (`engineering`)
- **Resumo:** Use após corrigir um bug para investigar a causa raiz sistêmica (5 Whys, Fishbone, Premortem) e recomendar mudanças de processo.
- **Ativar quando:** Use após corrigir um bug para investigar a causa raiz sistêmica (5 Whys, Fishbone, Premortem) e recomendar mudanças de processo.
- **Escopo canônico:** Skill: Root Cause Analyzer
- **Seções do corpo original:** Identity · Goals · Triggers · Techniques · Root Cause Categories
...
### Passo 1 — The null check pattern isn't applied consistently

The null check pattern isn't applied consistently

### Passo 2 — Other Str::

Other Str:: methods also changed behavior

### Passo 3 — Developers in other teams make the same assumption

Developers in other teams make the same assumption

### Passo 4 — The fix works but the underlying pattern repeats elsewhere

The fix works but the underlying pattern repeats elsewhere

Prevention:

### Passo 5 — Run codemod to add null checks everywhere

Run codemod to add null checks everywhere

### Passo 6 — Audit all Str::

Audit all Str:: usages in the codebase

### Passo 7 — Share findings in team tech talk

Share findings in team tech talk

### Passo 8 — Add to PR review checklist
