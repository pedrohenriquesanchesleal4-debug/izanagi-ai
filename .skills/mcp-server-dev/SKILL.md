---
name: "mcp-server-dev"
description: "Desenvolvimento de servidores MCP: tools com schema Zod, resources, prompts, transportes STDIO/HTTP e sanitização contra path-traversal e command injection. Use ao criar ou estender servidores MCP. Gatilhos de ativação: model context protocol (mcp) server development; quando usar; conceitos essenciais (o que o servidor expõe); stack de implementação (typescript padrão)."
version: 2.0.0
category: ai
tools:
  mcp:
...
> Migrado deterministicamente de `skills/mcp-server-dev/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** IA & Agentes (`ai`)
- **Resumo:** Desenvolvimento de servidores MCP: tools com schema Zod, resources, prompts, transportes STDIO/HTTP e sanitização contra path-traversal e command injection.
- **Ativar quando:** Use ao criar ou estender servidores MCP.
- **Escopo canônico:** Model Context Protocol (MCP) Server Development
- **Seções do corpo original:** Quando usar · Conceitos essenciais (o que o servidor expõe) · Stack de implementação (TypeScript padrão) · Workflow de desenvolvimento (6 passos) · Checklist de qualidade (antes de entregar)
...
### Passo 1 — Passo 1 — Escopo das tools (comece pequeno)
...
### Passo 2 — Passo 2 — Schemas estritos (nunca confie no input)
...
### Passo 3 — Passo 3 — Implemente a tool com erro estruturado
...
### Passo 4 — Passo 4 — Sanitização obrigatória (segurança)
...
### Passo 5 — Passo 5 — Transportes (STDIO + HTTP/SSE)
...
### Passo 6 — Passo 6 — Teste e documente
