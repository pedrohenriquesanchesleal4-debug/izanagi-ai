...
description: "Padrões arquiteturais (Clean Architecture, Hexagonal, DDD, CQRS, Event-Driven, Microservices) com matriz de decisão, trade-offs e template de ADR. Use ao projetar ou refatorar a arquitetura do sistema. Gatilhos de ativação: architecture patterns — manual operacional; quando usar; matriz de decisão: qual padrão usar; clean architecture (robert c. martin)."
version: 2.0.0
...
> Migrado deterministicamente de `skills/architecture-patterns/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Engenharia (`engineering`)
- **Resumo:** Padrões arquiteturais (Clean Architecture, Hexagonal, DDD, CQRS, Event-Driven, Microservices) com matriz de decisão, trade-offs e template de ADR.
- **Ativar quando:** Use ao projetar ou refatorar a arquitetura do sistema.
- **Escopo canônico:** Architecture Patterns — Manual Operacional
- **Seções do corpo original:** Quando usar · Matriz de Decisão: Qual Padrão Usar · Clean Architecture (Robert C. Martin) · Hexagonal Architecture (Ports & Adapters) · DDD (Domain-Driven Design)
...
**Core puro**: Regras de negócio sem NENHUMA dependência externa (nem framework, nem banco, nem HTTP).

### Passo 2 — Ports = interfaces:

**Ports = interfaces**: O core define contratos (`UserRepository`, `EmailSender`). Adapters implementam.

### Passo 3 — Adapters trocáveis:

**Adapters trocáveis**: Trocar PostgreSQL por MongoDB = trocar só o adapter, sem tocar no core.

### Passo 4 — Testabilidade:

**Testabilidade**: Core testável com mocks dos ports. Zero infra nos testes unitários.
