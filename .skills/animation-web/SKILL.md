---
name: "animation-web"
description: "Scrollytelling, scroll-driven animations, sequências de imagem em canvas (estilo Apple), parallax e pinned sections. Use quando o site não deve parecer estático e o scroll for a timeline da experiência. Gatilhos de ativação: animation web — scrollytelling & cinematic sites; identity; princípios; arquitetura de técnicas (decision tree)."
version: 2.0.0
category: design
tools:
  mcp:
...
> Migrado deterministicamente de `skills/animation-web/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Design & UI (`design`)
- **Resumo:** Scrollytelling, scroll-driven animations, sequências de imagem em canvas (estilo Apple), parallax e pinned sections.
- **Ativar quando:** Use quando o site não deve parecer estático e o scroll for a timeline da experiência.
- **Escopo canônico:** Animation Web — Scrollytelling & Cinematic Sites
- **Seções do corpo original:** Identity · Princípios · Arquitetura de Técnicas (Decision Tree) · Workflow · Padrões Core
...
### Passo 1 — Storyboard primeiro.

**Storyboard primeiro.** Divida o conteúdo em cenas (hero → capítulo 1..n → finale). Cada cena = 1 técnica + 1 gatilho de scroll.

### Passo 2 — Escolha a stack de scroll:

**Escolha a stack de scroll**: Lenis (smooth scroll) + GSAP ScrollTrigger (controle de scrub/pin) é o padrão; React usa `lenis` + `@gsap/react` (`useGSAP`).

### Passo 3 — Implemente por cena, começando pelo hero.

**Implemente por cena**, começando pelo hero.

### Passo 4 — Valide perf:

**Valide perf**: DevTools Performance panel, Lighthouse, Core Web Vitals (LCP/INP/CLS).

### Passo 5 — Degrade:

**Degrade**: sem JS → conteúdo estático visível; reduced motion → sem scrub.
