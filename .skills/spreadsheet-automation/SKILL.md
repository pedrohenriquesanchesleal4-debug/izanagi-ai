...
description: "Use para automatizar planilhas (Excel, CSV, ODS, Google Sheets) como fonte ou destino de dados: leitura, validação, normalização e escrita sem perda de dados. Gatilhos de ativação: spreadsheet automation — planilhas como fonte de dados confiável; quando usar; stack (escolha por necessidade real); workflow obrigatório (5 passos)."
version: 2.0.0
category: data
tools:
  mcp:
...
> Migrado deterministicamente de `skills/spreadsheet-automation/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Dados (`data`)
- **Resumo:** Use para automatizar planilhas (Excel, CSV, ODS, Google Sheets) como fonte ou destino de dados: leitura, validação, normalização e escrita sem perda de dados.
- **Ativar quando:** Use para automatizar planilhas (Excel, CSV, ODS, Google Sheets) como fonte ou destino de dados: leitura, validação, normalização e escrita sem perda de dados.
- **Escopo canônico:** Spreadsheet Automation — Planilhas como Fonte de Dados Confiável
- **Seções do corpo original:** Quando usar · Stack (escolha por necessidade real) · Workflow obrigatório (5 passos) · Anti-padrões (proibido) · Checklist de qualidade (antes de entregar)
...
### Passo 1 — Passo 1 — Inspecione antes de processar (nunca pule)
...
### Passo 2 — Passo 2 — Defina o schema e valide cedo (Pydantic)
...
### Passo 3 — Passo 3 — Normalize com regras explícitas
...
### Passo 4 — Passo 4 — Duplicados: marque, não delete silenciosamente
...
### Passo 5 — Passo 5 — Escreva sem destruir a fonte
