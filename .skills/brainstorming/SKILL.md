---
name: "brainstorming"
description: "Transforma uma ideia bruta em design aprovado por entrevista dirigida (~15 perguntas), referências e blueprint de arquitetura. Use antes de criar features ou componentes — hard-gate até o design ser aprovado. Gatilhos de ativação: brainstorming — da ideia ao design aprovado; hard gate (innegociável); método star; triagem por peso da tarefa (antes da entrevista completa)."
version: 2.0.0
...
> Migrado deterministicamente de `skills/brainstorming/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Engenharia (`engineering`)
- **Resumo:** Transforma uma ideia bruta em design aprovado por entrevista dirigida (~15 perguntas), referências e blueprint de arquitetura.
- **Ativar quando:** Use antes de criar features ou componentes — hard-gate até o design ser aprovado.
- **Escopo canônico:** Brainstorming — Da Ideia ao Design Aprovado
- **Seções do corpo original:** Hard Gate (innegociável) · Método STAR · Triagem por peso da tarefa (antes da entrevista completa) · Entrevista em 3 Fases (~15 perguntas, UMA por vez — nunca dump) · Referências — 2 Trilhas OBRIGATÓRIAS
...
### Passo 1 — Explore o contexto do projeto — arquivos, docs, commits recentes, stack

**Explore o contexto do projeto** — arquivos, docs, commits recentes, stack

### Passo 2 — Perguntas de esclarecimento — 3 fases, UMA por vez, múltipla escolha quando possível

**Perguntas de esclarecimento** — 3 fases, UMA por vez, múltipla escolha quando possível

### Passo 3 — 2 Trilhas de referências — visual + técnica, com URLs reais e porquês

**2 Trilhas de referências** — visual + técnica, com URLs reais e porquês

### Passo 4 — Proponha 2–3 abordagens — com trade-offs e sua recomendação primeiro

**Proponha 2–3 abordagens** — com trade-offs e sua recomendação primeiro

### Passo 5 — Arquitetura & Blueprint — diretórios, stack, dados, endpoints, ADR-lite

**Arquitetura & Blueprint** — diretórios, stack, dados, endpoints, ADR-lite

### Passo 6 — Apresente o design em seções — escale ao tamanho do problema; consiga aprovação após ca...

**Apresente o design em seções** — escale ao tamanho do problema; consiga aprovação após cada seção

### Passo 7 — Confirme o norte (1 pergunta) — HARD-GATE

**Confirme o norte** (1 pergunta) — HARD-GATE

### Passo 8 — Escreva o design doc / prompt rico — docs/superpowers/specs/YYYY-MM-DD-<tema-design.md...

**Escreva o design doc / prompt rico** — `docs/superpowers/specs/YYYY-MM-DD-<tema>-design.md` (ou prompt rico do Discovery) e commite

### Passo 9 — Auto-revisão do spec — placeholders?

**Auto-revisão do spec** — placeholders? contradições? ambiguidade? escopo? Corrija inline

### Passo 10 — Devolva ao usuário — "Spec escrito em <path.

**Devolva ao usuário** — "Spec escrito em `<path>`. Revisa antes de planejarmos a implementação?"

### Passo 11 — Transição — aponte para a escrita do plano de implementação (no Izanagi:

**Transição** — aponte para a escrita do plano de implementação (no Izanagi: `task-planner` / agente relevante)
