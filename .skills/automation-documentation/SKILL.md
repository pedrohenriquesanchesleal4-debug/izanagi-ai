...
description: "Gera README de automações: instalação, configuração (.env), execução com dry-run, testes, limitações e troubleshooting. Use em toda automação entregue, para que outra pessoa consiga executar e manter. Gatilhos de ativação: automation documentation; quando usar; template de readme (seções obrigatórias); pré-requisitos."
version: 2.0.0
category: docs
tools:
  mcp:
...
> Migrado deterministicamente de `skills/automation-documentation/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Documentação & Comunicação (`docs`)
- **Resumo:** Gera README de automações: instalação, configuração (.env), execução com dry-run, testes, limitações e troubleshooting.
- **Ativar quando:** Use em toda automação entregue, para que outra pessoa consiga executar e manter.
- **Escopo canônico:** Automation Documentation
- **Seções do corpo original:** Quando usar · Template de README (seções obrigatórias) · Pré-requisitos · Instalação · Configuração (.env)
...
### Passo 1 — Lê planilha → 2.

Lê planilha → 2. valida schema → 3. envia via API → 4. grava relatório em output/
