---
name: "webgl-3d"
description: "Cenas 3D no navegador com Three.js/React Three Fiber, shaders GLSL, partículas, GLTF e scroll-driven 3D, com budget de performance. Use quando o pedido envolver 3D, WebGL, shader ou experiência imersiva. Gatilhos de ativação: webgl 3d — three.js & react three fiber; identity; decisão de stack (decision tree); workflow."
version: 2.0.0
...
> Migrado deterministicamente de `skills/webgl-3d/SKILL.md` (v1) pelo skill-migrator (ADR-004). Corpo original preservado íntegro em **Legacy Reference (v1)**.

## Triggering Criteria

- **Domínio:** Engenharia (`engineering`)
- **Resumo:** Cenas 3D no navegador com Three.js/React Three Fiber, shaders GLSL, partículas, GLTF e scroll-driven 3D, com budget de performance.
- **Ativar quando:** Use quando o pedido envolver 3D, WebGL, shader ou experiência imersiva.
- **Escopo canônico:** WebGL 3D — Three.js & React Three Fiber
- **Seções do corpo original:** Identity · Decisão de stack (Decision Tree) · Workflow · Padrões Core · Rules (Performance)
...
### Passo 1 — Scene design primeiro:

**Scene design primeiro**: o que é a cena (objeto, câmera, luz, fundo)? Quantas cenas (uma por seção)?

### Passo 2 — Setup base:

**Setup base**: `npm i three` + (`@react-three/fiber @react-three/drei` p/ React). No Vite: `optimizeDeps: { include: ['three'] }`.

### Passo 3 — Modelos:

**Modelos**: exportar GLB + Draco; texturas KTX2/Basis; carregar com suspense e fallback de loading.

### Passo 4 — Integrar com scroll/DOM:

**Integrar com scroll/DOM**: câmera e objetos reagem a `ScrollTrigger` (scrub) ou a overlays HTML por cima do canvas fixo.

### Passo 5 — Performance budget (obrigatório):

**Performance budget** (obrigatório): DPR cap, render só quando visível, desligar sombras pesadas no mobile.

### Passo 6 — Fallback + reduced motion:

**Fallback + reduced motion**: imagem estática ou cena simplificada.
